...
\begin{tikzpicture}[
  node distance=0.6cm,
  stepbox/.style={draw, rounded corners=2pt, minimum width=2.8cm, minimum height=0.55cm, font=\scriptsize, align=center, fill=primary!8, draw=primary!50},
  arr/.style={-{Stealth[length=3pt]}, thick, primary!70},
]

\node[stepbox] (input) {Image Input\\(file, base64, clipboard)};
\node[stepbox, below=of input] (decode) {Decode \& Validate\\(PNG, JPEG, WebP, GIF)};
\node[stepbox, below=of decode] (thumb) {Thumbnail Generation\\(Lanczos3, max 512px, Q85)};
\node[stepbox, below=of thumb] (embed) {CLIP Embedding\\(224$\times$224, ViT-B/32, 512-d)};
\node[stepbox, below=of embed] (meta) {Metadata Assembly\\(dims, labels, description)};
\node[stepbox, below=of meta, fill=secondary!10, draw=secondary!50] (store) {Store in \texttt{.avis}\\(header + JSON payload)};
